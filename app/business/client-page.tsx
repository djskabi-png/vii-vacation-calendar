...
export default function BusinessPage({ initialSlug, initialWorld = "vacation", initialDates, initialFrom, initialTill, initialGuests = "2", initialPrice }: { initialSlug: string; initialWorld?: BusinessWorld; initialDates?: string; initialFrom?: string; initialTill?: string; initialGuests?: string; initialPrice?: string }) {
  ...
  const bookingQuery = new URLSearchParams({ world: activeWorld, place: property.slug, ...(dateRange.from ? { from: dateRange.from } : {}), ...(dateRange.till ? { till: dateRange.till } : {}), ...(initialGuests ? { guests: initialGuests } : {}), ...(initialPrice ? { price: initialPrice } : {}) }).toString();
      ...
      <main id="main-content" className="property-page">
        ...
        <section className="shell property-title">
          ...
          <div className="property-title__side">
            ...
            {onlineBooking ? <Link className="button primary" href={`/booking?${bookingQuery}`}>הזמנה אונליין</Link> : property.contact?.phone ? <a className="button primary" href={`tel:${property.contact.phone.replace(/[^\d+]/g, "")}`}>הזמנה בטלפון</a> : null}
          </div>
        </section>
        ...
        <DetailStickyDock
          ...
          onlineHref={onlineBooking ? `/booking?${bookingQuery}` : undefined}
          onlineLabel={activeWorld === "events" ? "בדיקת תאריך לאירוע" : "בדיקת זמינות"}
              ...
              <div className="room-card-list">
                {property.roomOptions.map((room) => <article className="room-card" key={room.name}>
                  ...
                  <div className="room-card__body">
                    ...
                    <div className="room-card__actions">{onlineBooking ? <button className="button primary" type="button" onClick={() => setCalendarOpen(true)}>בדיקת זמינות</button> : phoneHref ? <a className="button primary" href={phoneHref}>הזמנה בטלפון</a> : null}</div>
                  </div>
                </article>)}
              </div>
            </section> : null}
