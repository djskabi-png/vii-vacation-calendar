      ...
      <main id="main-content" className="property-page">
        ...
        {property.demoOperations?.fictional && property.videos?.[0] ? <section className="shell palumbo-media-story" aria-labelledby="palumbo-media-title">
          ...
          <div className="palumbo-media-story__grid">
            ...
            <aside className="palumbo-media-story__guests"><div><span>גלריית אורחים</span><strong>רגעים מהאירוח</strong><p>תמונות אורחים מוצגות לאחר אימות ואישור לפרסום.</p></div><div>{property.guestPhotos?.map((photo) => <button type="button" key={photo.src} onClick={() => { setGalleryTab("guests"); setGalleryStart(0); setGalleryOpen(true); }}><img src={photo.src} alt={photo.alt} /><span>{photo.author}</span></button>)}</div></aside>
          </div>
        </section> : null}
