  ...
  return (
    <PageShell variant={activeWorld}>
      <main id="main-content" className="property-page">
        ...
        {property.demoOperations?.fictional ? <section className="shell fictional-property-notice" role="note"><strong>מתחם בדיקה בדיוני</strong><span>השם, הכתובת וכל התמונות הומצאו לצורך בדיקת המוצר. התמונות הן הדמיות שנוצרו בבינה מלאכותית ואינן מתעדות מקום אמיתי. אין להגיע לכתובת.</span></section> : null}
        ...
        {property.demoOperations?.fictional && property.videos?.[0] ? <section className="shell palumbo-media-story" aria-labelledby="palumbo-media-title">
          <header><div><span className="eyebrow">הסיפור המלא של המקום</span><h2 id="palumbo-media-title">רואים את הווילה לפני שבוחרים</h2><p>סיור, חדרים ותמונות אורחים לדוגמה, כולם מסומנים כהמחשה בדיונית שנוצרה לצורך בדיקת העמוד.</p></div><button type="button" className="button secondary" onClick={() => { setGalleryTab("all"); setGalleryStart(0); setGalleryOpen(true); }}>פתיחת הגלריה המלאה</button></header>
          <div className="palumbo-media-story__grid">
            <article className="palumbo-media-story__video"><video controls playsInline preload="metadata" poster={property.videos[0].poster} aria-label={property.videos[0].title}><source src={property.videos[0].src} type="video/mp4" /></video><div><span>סרטון המחשה</span><h3>{property.videos[0].title}</h3><p>{property.videos[0].note}</p></div></article>
            <aside className="palumbo-media-story__guests"><div><span>תמונות אורחים לדוגמה</span><strong>כך נראית חוויית התוכן של האורחים</strong><p>המצולמים, השמות והחוויות אינם אמיתיים. התמונות נועדו להמחיש גלריית אורחים לאחר אימות ומודרציה.</p></div><div>{property.guestPhotos?.map((photo) => <button type="button" key={photo.src} onClick={() => { setGalleryTab("guests"); setGalleryStart(0); setGalleryOpen(true); }}><img src={photo.src} alt={photo.alt} /><span>{photo.author}</span></button>)}</div></aside>
          </div>
        </section> : null}
