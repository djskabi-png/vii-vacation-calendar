...
export default function BusinessPage({ initialSlug, initialWorld = "vacation", initialDates, initialFrom, initialTill, initialGuests = "2", initialRooms = "1", initialPrice, initialSource }: { initialSlug: string; initialWorld?: BusinessWorld; initialDates?: string; initialFrom?: string; initialTill?: string; initialGuests?: string; initialRooms?: string; initialPrice?: string; initialSource?: string }) {
  ...
  const hasSelectedPrice = Boolean(initialPrice && Number(initialPrice) > 0);
    ...
    <PageShell variant={activeWorld}>
      <main id="main-content" className="property-page">
        ...
        {hasSearchContext && activeWorld === "vacation" ? <section className="search-context-summary shell" aria-label="החיפוש שממנו הגעתם">
          ...
          <Link className="button primary" href="#booking-summary">בדיקת זמינות לחיפוש הזה</Link>
        </section> : null}
        ...
        <DetailStickyDock
          ...
          onlineHref={onlineBooking || vacationPhoneFallback ? bookingActionHref : undefined}
          onlineLabel={activeWorld === "events" ? "בדיקת תאריך לאירוע" : activeWorld === "vacation" ? hasSearchContext ? "בדיקת זמינות לחיפוש הזה" : "בדיקת זמינות" : "הזמנה אונליין"}
              ...
              <div className="room-card-list">
                {property.roomOptions.map((room) => <article className="room-card" key={room.name}>
                  ...
                  <div className="room-card__body">
                    ...
                    <div className="room-card__actions">{activeWorld === "vacation" ? <Link className="button primary" href="#booking-summary">{hasSearchContext ? "בדיקת זמינות לחיפוש הזה" : "בדיקת זמינות"}</Link> : onlineBooking ? <Link className="button primary" href={bookingActionHref}>הזמנה אונליין</Link> : phoneHref ? <Link className="button primary" href="#booking-summary">טלפון להזמנה</Link> : null}</div>
                  </div>
                </article>)}
              </div>
            </section> : null}
