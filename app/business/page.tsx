  ...
  return (
    <PageShell>
      <main id="main-content" className="property-page">
        ...
        <section className={`shell property-gallery ${property.images.length === 1 ? "single" : ""}`}>{property.images.slice(0,5).map((image,index) => <button key={image} type="button" onClick={() => setGalleryOpen(true)}><img src={image} alt={`${property.name}, תמונה ${index+1}`} />{index === Math.min(4, property.images.length-1) && property.images.length > 1 && <span>לכל התמונות</span>}</button>)}</section>
