...
export function AccountHeaderButton() { const { language } = useSiteLanguage(); const { account, openLogin } = useAccountAccess(); return account ? <Link className="account-header-button account-header-button--connected" href="/account" aria-label={copy[language].account}><span className="account-header-avatar" aria-hidden="true">{account.name.trim().slice(0, 1).toUpperCase()}</span><span>{account.name.split(" ")[0]}</span></Link> : <button className="account-header-button" type="button" onClick={openLogin}><AccountUserIcon /><span>{copy[language].login}</span></button>; }
export function AccountFormPrompt() { const { language } = useSiteLanguage(); const { account, openLogin } = useAccountAccess(); const labels = copy[language]; return account ? <div className="account-form-prompt account-form-prompt--connected" role="status"><span className="account-header-avatar" aria-hidden="true">{account.name.trim().slice(0, 1).toUpperCase()}</span><strong>{labels.connected}</strong></div> : <div className="account-form-prompt"><span><strong>{labels.promptTitle}</strong><small>{labels.promptBody}</small></span><button type="button" onClick={openLogin}>{labels.promptAction}</button></div>; }