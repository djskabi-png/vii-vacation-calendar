...
export function PageShell({ children, variant = "vacation", showWorldSwitcher = true }: { children: ReactNode; variant?: WorldId; showWorldSwitcher?: boolean }) {
  return <div className="site-page"><a className="skip-link" href="#main-content">דילוג לתוכן</a><SiteHeader variant={variant} />{children}<SiteFooter />{showWorldSwitcher ? <WorldSwitcher active={variant} /> : null}<SmartConcierge /><CookieConsent /></div>;
}
