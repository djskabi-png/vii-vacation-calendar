...
export function WorldSwitcher({ active = "vacation" }: { active?: WorldId }) {
  ...
  return (
    <div className={`world-dock ${open ? "open" : ""}`}>
      ...
      <button type="button" aria-expanded={open} aria-haspopup="true" onClick={() => setOpen((value) => !value)}><span className={`world-mark world-mark--${current.id}`} aria-hidden="true" /><span><small>{current.shortLabel}</small><strong>{open ? "סגירה" : "עוברים עולם"}</strong></span></button>
    </div>
  );
}

export function SearchWorldTabs({ active }: { active: WorldId }) {
  return <nav className="search-world-tabs" aria-label="בחירת עולם לחיפוש">{worlds.slice(0, 4).map((world) => <Link key={world.id} href={world.href} className={world.id === active ? "active" : ""}><span className={`world-mark world-mark--${world.id}`} aria-hidden="true" />{world.shortLabel}</Link>)}</nav>;
}
