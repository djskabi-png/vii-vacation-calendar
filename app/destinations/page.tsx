...
export default function DestinationsPage() {
  return <PageShell><main id="main-content">
    ...
    <section className="section shell destination-directory">{destinations.map((item, index) => <article key={item.name}><img src={item.image} alt={item.name} /><div><span>0{index + 1}</span><h2>{item.name}</h2><p>{item.subtitle}</p><Link className="button secondary" href={destinationPath(item.name)}>למקומות באזור</Link></div></article>)}</section>
