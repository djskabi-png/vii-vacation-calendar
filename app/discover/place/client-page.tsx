...
function HourlyContent({ itemId, details }: { itemId: string; details: HourlyDetails }) {
  return <>
    ...
    <section className="section depth-final-cta"><div className="shell"><div><span className="eyebrow">מוכנים להזמין?</span><h2>בוחרים שעה ומשך שהייה</h2><p>ההזמנה כוללת את המקום, משך השהייה וכל בקשה מיוחדת.</p></div><Link className="button primary" href={`/booking?world=hourly&place=${encodeURIComponent(itemId)}`}>להזמנה אונליין</Link></div></section>
  </>;
}
...
export default function DiscoveryPlacePage({ initialId }: { initialId: string }) {
  ...
  const phone = activityPhoneOnly ? activityDetails.booking?.phone : undefined;
  const onlineHref = activityPhoneOnly ? undefined : item.world === "spa" && spaDetails?.packages?.length
    ? "#spa-packages"
    : `/booking?world=${item.world}&place=${encodeURIComponent(item.id)}`;
  const onlineLabel = item.world === "spa" ? "בחירת חבילת ספא" : item.world === "hourly" ? "בחירת שעה" : item.world === "providers" ? "בחירת שירות" : "הזמנה אונליין";
