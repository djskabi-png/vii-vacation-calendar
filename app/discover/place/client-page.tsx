...
function ActivityContent({ itemId, details }: { itemId: string; details: ActivityDetails }) {
  const onlineHref = `/contact?world=activities&place=${encodeURIComponent(itemId)}&intent=activity-order`;
  const phoneHref = details.booking?.phone ? `tel:${details.booking.phone.replace(/[^\d+]/g, "")}` : "";
  return <>
    <nav className="shell depth-detail-nav" aria-label="ניווט בדף הפעילות"><a href="#activity-about">על החוויה</a><a href="#activity-highlights">מה מחכה בדרך</a><a href="#activity-plan">תכנון מלא</a><a href="#activity-preparation">מה להכין</a>{details.booking ? <a href="#activity-booking">איך מזמינים</a> : null}<a href="#activity-faq">שאלות נפוצות</a></nav>
    ...
    <section className="section depth-final-cta"><div className="shell"><div><span className="eyebrow">משלבים את זה בחופשה</span><h2>{details.kind === "matching" ? "נמצא חוויה שמתאימה בדיוק להרכב שלכם" : "שומרים את התוכנית וממשיכים לבחור מקום אירוח"}</h2><p>{details.kind === "matching" ? "דרך ההזמנה נקבעת לפי הספק: רכישה באתר, חיוג ישיר או שתי האפשרויות." : "אפשר לחזור לחיפוש ולבחור מקום באזור שמתאים למסלול."}</p></div>{details.booking ? <div className="activity-booking__actions">{details.booking.mode !== "phone" ? <Link className="button primary" href={onlineHref}>{details.booking.onlineLabel || "רכישה אונליין"}</Link> : null}{details.booking.mode !== "online" && phoneHref ? <a className="button secondary" href={phoneHref}>חיוג למקום</a> : null}</div> : <Link className="button primary" href={details.kind === "matching" ? `/contact?world=activities&place=${encodeURIComponent(itemId)}` : "/"}>{details.kind === "matching" ? "בקשת התאמה" : "לחיפוש מקום אירוח"}</Link>}</div></section>
  </>;
}
