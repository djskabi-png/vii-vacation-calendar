  ...
  return (
    <PageShell variant="events">
      <main id="main-content">
        <section className="events-hero"><div className="shell"><span className="eyebrow">אירוע שמרגיש בדיוק שלכם</span><h1>מוצאים מקום לחגוג בו</h1><p>מחפשים לפי אזור, תאריך וכמות משתתפים ומשווים בין מקומות אמיתיים.</p><SearchBox mode="events" /></div></section>
        ...
        <section className="section section-tint"><div className="shell"><div className="section-head"><div><span className="eyebrow">מקומות שכדאי להכיר</span><h2>מומלצים לאירוע הבא</h2></div><Link href="/events/search/">לכל המקומות</Link></div><div className="event-card-grid">{eventPlaces.slice(0, 4).map((place) => <article className="event-card" key={place.slug}><Link href={`/events/place/?id=${place.slug}`}><img src={place.image} alt={place.name} /><span className="event-card__body"><small>{place.type}</small><h3>{place.name}</h3><p><PinIcon />{place.location}</p><span>עד {place.guests} אורחים</span><span className="button secondary">לפרטים</span></span></Link></article>)}</div></div></section>
