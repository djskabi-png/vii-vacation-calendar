    ...
    <PageShell>
      <main id="main-content">
        ...
        <section className="section shell">
          ...
          {savedEvents.length > 0 && <><div className="section-head favorites-event-head"><h2>אירועים</h2></div><div className="event-more-grid">{savedEvents.map((place) => <Link key={place.slug} href={eventPlaceHref(place)}><img src={place.image} alt={place.name} /><div><b>{place.name}</b><span>{place.location} · עד {place.guests} אורחים</span></div></Link>)}</div></>}
