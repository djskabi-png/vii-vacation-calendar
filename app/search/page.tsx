...
export function SearchExperience({ landing }: { landing?: SearchLandingContext }) {
  ...
  function currentFilterState(): VacationFilterState {
    return { area, type, guests, pool, spa, whole, accessibleOnly, selectedExtras, sort };
  }
  ...
  function changeType(nextType: string) {
    if (filtersOpen && draftFilters) {
      setDraftFilters({ ...draftFilters, type: nextType });
      return;
    }
    setType(nextType);
  ...
  const draftCandidates = properties.filter((property) => {
    const legacyType = legacyAccommodationTypes.find((item) => item.label === shownFilters.type);
    const matchesType = shownFilters.type === "הכל" || (landing?.types?.length && shownFilters.type === landing.type ? landing.types.includes(property.type) : legacyType ? legacyType.matches.some((item) => item === property.type) : property.type === shownFilters.type);
    const searchableFacts = [property.description, property.type, property.location, property.area, ...property.features].join(" ").toLocaleLowerCase("he");
  ...
  const draftUsesLandingSet = Boolean(landing?.listingSlugs?.length && (landing.area ? shownFilters.area === landing.area : shownFilters.area === "הכל"));
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [area, type, guests, pool, spa, whole, accessibleOnly, selectedExtras, sort]);
  ...
  function resetFilters() {
    if (filtersOpen) {
      setDraftFilters({ area: "הכל", type: "הכל", guests: 2, pool: false, spa: false, whole: false, accessibleOnly: false, selectedExtras: [], sort: "recommended" });
      return;
    }
    setArea("הכל");
    setType("הכל");
  ...
  function applyFilters() {
    ...
    updateSearchContext({
      ...
      type: draftFilters.type === "הכל" ? null : draftFilters.type,
    ...
    }, draftFilters.type, draftFilters.area);
    closeFiltersPanel();
  }
        ...
        <div className={`shell results-layout ${mapOpen ? "with-map" : ""}`}>
          ...
          <section className="results-list" aria-label="תוצאות">
            <section className="results-heading">
              <div><h1>{landing?.title || (area === "הכל" ? "נופש ברחבי הארץ" : `נופש ב${area}`)}</h1><div className="results-heading__meta"><p>{mapVisibleIds && !mapOpen ? `${displayedResults.length} מקומות באזור שבחרתם במפה` : landingResultSummary || (mapOpen ? `${visibleMapCount} מקומות באזור המוצג במפה` : area === "הכל" ? `נמצאו ${displayedResults.length} מקומות` : `נמצאו ${displayedResults.length} מקומות ב${area}`)}</p><button type="button" className={`mobile-filter mobile-filter--compact ${activeFilters.length ? "has-filters" : ""}`} aria-label="סינון" aria-expanded={filtersOpen} onClick={(event) => { event.preventDefault(); event.stopPropagation(); openFiltersPanel(); }}><span className="mobile-filter__icon" aria-hidden="true"><i /><i /><i /></span>{activeFilters.length ? <b aria-label={`${activeFilters.length} סינונים פעילים`}>{activeFilters.length}</b> : null}</button></div></div>
            </section>
            ...
            {!mapOpen && <div className="result-cards">{displayedResults.map((property) => <PropertyCard key={property.slug} property={property} />)}</div>}
