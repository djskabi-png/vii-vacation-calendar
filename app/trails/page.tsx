...
export default function TrailsPage() {
  return <PageShell variant="activities">
    <main id="main-content" className="trails-page">
      ...
      <StructuredData data={breadcrumbSchema([{ name: "ראשי", path: "/" }, { name: "מה עושים בחופשה", path: "/activities" }, { name: "מסלולי טיול", path: "/trails" }])} />
      <BreadcrumbTrail className="world-breadcrumbs" items={[{ name: "ראשי", path: "/" }, { name: "מה עושים בחופשה", path: "/activities" }, { name: "מסלולי טיול" }]} />
      <section className="trails-hero"><div className="shell trails-hero__layout"><div><span className="eyebrow">מסלולי טיול שהופקו במיוחד למטיילי וי פור ויקיישן</span><h1>יוצאים מהצימר. נכנסים לישראל היפה.</h1><p>מפלים, נהרות, מעיינות, חופים, יערות ומדבר. בוחרים לפי אזור, זמן ודרגת קושי ומקבלים תוכנית טיול ברורה עם מקור רשמי.</p><div className="trails-hero__actions"><a className="button primary" href="#trail-search">מציאת מסלול</a><Link className="button light" href="/attractions">גם אטרקציות בתשלום</Link></div></div><div className="trails-hero__map" aria-hidden="true"><span>צפון</span><i /><i /><i /><strong>{trails.length}</strong><small>מסלולים מוכנים</small><b>דרום</b></div></div></section>
