...
test("mobile business details place bookable units before the long facilities inventory", () => {
  ...
  assert.match(source, /<details className="feature-section__mobile-details">/);
  assert.match(source, /<summary>כל המידע על המתקנים<\/summary>/);
...
test("mobile facilities use progressive disclosure while desktop keeps the complete groups", () => {
  ...
  assert.match(css, /\.feature-section__mobile-details summary:focus-visible/);
}
);
