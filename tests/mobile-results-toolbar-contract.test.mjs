...
test("mobile vacation and event headings keep a compact filter beside the result count", async () => {
  ...
  sources.forEach((source) => {
    ...
    assert.doesNotMatch(source, /mobile-filter[^>]*>[\s\S]{0,180}<span>סינון<\/span>/);
  ...
  assert.match(css, /\.results-heading__meta \.mobile-filter \{[\s\S]*width: 40px;[\s\S]*min-height: 40px;[\s\S]*border-radius: 50%;[\s\S]*background: #fff/);
